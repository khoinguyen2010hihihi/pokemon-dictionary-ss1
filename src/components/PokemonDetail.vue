...
const pokemonId = computed(() => {
  return selectedPokemon.value ? getIDPokemon(selectedPokemon.value.species.url) : null;
});
...
async function fetchPokemonDetails(pokemonName) {
  try {
    ...
    const pokemonData = await fetchAPI(`https://pokeapi.co/api/v2/pokemon/${pokemonName}`);
    selectedPokemon.value = pokemonData;
    pokemonTypes.value = pokemonData.types.map((type) => type.type.name);

    const speciesData = await fetchAPI(`https://pokeapi.co/api/v2/pokemon-species/${pokemonName}`);
    flavorText.value = speciesData.flavor_text_entries.find((entry) => entry.language.name === 'en')?.flavor_text || '';

    const evolutionChainUrl = speciesData.evolution_chain?.url;
    if (evolutionChainUrl) {
      const evolutionChainData = await fetchAPI(evolutionChainUrl);
      selectedPokemon.value.evolutionChain = extractEvolutionChain(evolutionChainData.chain);
    }
